  ...
  return (
    <section className="mt-6 rounded-[2rem] border border-amber-500/30 bg-amber-950/20 p-6">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-4">
        ...
        <Info
          label="Temperature"
          value={chainClimate ? `${chainClimate.temperature}°C` : climate ? `${climate.temperature}°C` : "Loading"}
        />
        <Info
          label="Humidity"
          value={chainClimate ? `${chainClimate.humidity}%` : climate ? `${climate.humidity}%` : "Loading"}
        />
        <Info
          label="Risk Level"
          value={chainClimate ? `${chainClimate.riskLevel}/5` : climate ? `${climate.riskLevel}/5` : "Loading"}
        />
      </div>
