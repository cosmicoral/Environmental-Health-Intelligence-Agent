...
function DecisionGate({ alert, climate, esg }) {
  ...
  const esgDecision =
    esg?.forecast >= esgThreshold ? "Trigger Alert" : "Monitor Only";

  const shouldPublish =
    publicHealthDecision === "Trigger Alert" ||
    climateDecision === "Trigger Alert" ||
    esgDecision === "Trigger Alert";
  ...
  return (
    <section className="mt-6 rounded-[2rem] border border-emerald-500/30 bg-emerald-950/20 p-6">
      ...
      <p className="mt-5 text-sm leading-6 text-slate-400">
        The agent evaluates AI-generated health risk, climate risk, and carbon
        intensity before deciding whether to publish an on-chain alert.
      </p>
    </section>
  );
}
